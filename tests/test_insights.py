...
def test_get_company_insights_success(setup_data):
    # Test successful retrieval of insights
    response = client.get("/companies/user2/insights")
    
    assert response.status_code == 200
    assert "response_rate" in response.json()
    assert "conversation_ratio" in response.json()
    assert "volume_metrics" in response.json()
    assert "average_response_time" in response.json()

def test_get_company_insights_no_data():
    # Test retrieval of insights when no data is available
